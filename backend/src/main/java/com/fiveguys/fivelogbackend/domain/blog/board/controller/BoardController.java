...
//        log.info("pagedBoards {}", pagedBoards.getContent().size());
    ...
    @GetMapping("/search")
    public ResponseEntity<ApiResponse<List<BoardSummaryDto>>> searchBoards(
            @ModelAttribute SearchRequest searchRequest,
            @RequestParam(value = "lastId", defaultValue = "0") Long lastId, // default 0, 첫 페이지 요청 시
            @PageableDefault(size=12, page = 0, sort = "id", direction = Sort.Direction.DESC) Pageable pageable) {

        Page<Board> boards = boardService.searchByTitleOrHashtagPaging(searchRequest.getTitle(), searchRequest.getHashtags(),lastId, pageable);

        List<BoardSummaryDto> boardSummaryDtos = boardService.getBoardSummaryDtos(boards.getContent());

        return ResponseEntity.ok(ApiResponse.success(boardSummaryDtos,"게시글 검색 성공"));
    }
